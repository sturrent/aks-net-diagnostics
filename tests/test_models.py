...
from aks_diagnostics.models import DiagnosticResult, Finding, FindingCode, Severity, VMSSInstance
...
class TestVMSSInstance(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        instance = VMSSInstance(
            vmss_name="aks-nodepool1-vmss", resource_group="MC_test_rg", instance_id="0", metadata={"zone": "1"}
        )
    ...
    def test_default_metadata(self):
        ...
        instance = VMSSInstance(vmss_name="test", resource_group="rg", instance_id="0")
...
class TestFinding(unittest.TestCase):
    ...
    def test_factory_methods(self):
        ...
        critical = Finding.create_critical(FindingCode.CLUSTER_OPERATION_FAILURE, "Critical error", "Fix immediately")
        self.assertEqual(critical.severity, Severity.CRITICAL)

        warning = Finding.create_warning(FindingCode.NSG_BLOCKING_TRAFFIC, "Warning message", "Review NSG rules")
        self.assertEqual(warning.severity, Severity.WARNING)

        info = Finding.create_info(FindingCode.API_ACCESS_RESTRICTED, "Info message", "No action needed")
...
class TestDiagnosticResult(unittest.TestCase):
    ...
    def test_to_dict(self):
        """Test conversion to dictionary"""
        finding = Finding.create_warning(FindingCode.UDR_CONFLICT, "Test finding", "Test recommendation")
