...
class ClusterDataCollector:
    ...
    def collect_cluster_info(self, cluster_name: str, resource_group: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(cluster, "status") and cluster.status:
                if hasattr(cluster.status, "additional_properties") and cluster.status.additional_properties:
                    ...
                    cluster_result["status"].update(normalize_dict_keys(cluster.status.additional_properties))
        ...
        try:
            ...
            agent_pools_list = list(self.sdk_client.aks_client.agent_pools.list(resource_group, cluster_name))
                        ...
                        vnets_map[vnet_name]["peerings"].append(
                            {
                                ...
                                "remoteVirtualNetwork": (
                                    peering.remote_virtual_network.id if peering.remote_virtual_network else ""
                                ),
    ...
    def collect_vmss_info(self, cluster_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for vmss in vmss_list:
            ...
            try:
                ...
                vmss_detail = self.sdk_client.compute_client.virtual_machine_scale_sets.get(mc_rg, vmss_name)
