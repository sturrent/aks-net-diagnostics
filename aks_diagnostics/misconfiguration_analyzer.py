...
class MisconfigurationAnalyzer:
    ...
    def _analyze_private_dns_issues(
        self, cluster_info: Dict[str, Any], private_dns_analysis: Dict[str, Any], findings: List[Dict[str, Any]]
    ) -> None:
    ...
    def _check_dns_server_vnet_links(self, zone_rg: str, zone_name: str, findings: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            links_list = list(self.sdk_client.privatedns_client.virtual_network_links.list(zone_rg, zone_name))
    ...
    def _check_private_dns_vnet_links(
        self, cluster_info: Dict[str, Any], private_dns_zone: str, findings: List[Dict[str, Any]]
    ) -> None:
    ...
    def _analyze_udr_issues(self, outbound_analysis: Dict[str, Any], findings: List[Dict[str, Any]]) -> None:
        ...
        if va_routes:
            if default_va_routes:
                ...
                if any(
                    "container_registry" in r.get("impact", {}).get("affectedTraffic", []) for r in default_va_routes
                ):
                ...
                azure_va_routes = [
                    r
                    for r in va_routes
                    if r.get("impact", {}).get("affectedTraffic", [])
                    and "azure_services" in r.get("impact", {}).get("affectedTraffic", [])
                ]
                if azure_va_routes:
                ...
                mcr_va_routes = [
                    ...
                    for r in va_routes
                    if r.get("impact", {}).get("affectedTraffic", [])
                    and "container_registry" in r.get("impact", {}).get("affectedTraffic", [])
                ]
                if mcr_va_routes:
        ...
        bgp_disabled_tables = [rt for rt in route_tables if rt.get("disableBgpRoutePropagation") is True]
    ...
    def _analyze_api_server_access_issues(
        self, api_server_access_analysis: Dict[str, Any], findings: List[Dict[str, Any]]
    ) -> None:
    ...
    def _analyze_connectivity_test_results(
        ...
        http_failures = [
            ...
            if any(keyword in t.get("test_name", "") for keyword in ["HTTP", "Connectivity", "curl"])
        ]
        if http_failures:
